...
class UserControllerTest {
    ...
    @Test
    @DisplayName("GET v1/users?name=Kakashi returns list with found object when First name exists")
    @Order(2)
    void findAll_ReturnsFoundUserInList_WhenFirstNameIsFound() throws Exception {
        BDDMockito.when(userData.getUsers()).thenReturn(userList);
        var response = fileUtils.readResourceFile("user/get-user-kakashi-first-name-200.json");
